...
export function AllDisliked({userId}: AllDislikedProps) {
  const [AllSaved] = api.dislike.getByUser.useSuspenseQuery(
    { createdBy: userId }
  );
  ...
  if (!AllSaved || AllSaved.length === 0) {
    return (
      <AlertComponent type={"info"} icon={false} title={"No Objects Liked"} />
    );
  }

  // Group the data by 'type'
  const groupByType = (AllSaved: Favorite[]) => {
    return AllSaved.reduce((acc: Record<string, Favorite[]>, item) => {
      if (!acc[item.type]) {
        acc[item.type] = [];
      }
  ...
  const groupedData = groupByType(AllSaved);

  if (AllSaved && Object.keys(groupedData).length === 1) {
    return (
      <ListContainer>
        {AllSaved.map((item: Favorite) => (
          <ListItem key={item.id}>
            ...
            <div className="flex flex-none items-center gap-x-4">
              <ActionsComponent actions={['favorite', 'like', 'dislike', 'remove']} data={{model: 'dislike', id: item.id, key: item.cuid, object: item.object, type: item.type, label: item.cuid}} />
            </div>            
          ...
          <ListContainer>
            {groupedData[type]?.map((item) => (
              <ListItem key={item.id}>
                ...
                <div className="flex flex-none items-center gap-x-4">
                    <ActionsComponent actions={['remove']} data={{model: 'dislike', id: item.id, key: item.cuid, object: item.object, type: item.type, label: item.cuid}} />
                </div>
              </ListItem>
            )) ?? null}
          </ListContainer>
