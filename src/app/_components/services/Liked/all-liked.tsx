...
export function AllLiked({userId}: AllSavedProps) {
  const [AllSaved] = api.like.getByUser.useSuspenseQuery(
    { createdBy: userId }
  );
        ...
        {AllSaved.map((item: Favorite) => (
          <ListItem key={item.id}>
            ...
            <div className="flex flex-none items-center gap-x-4">
              <ActionsComponent actions={['favorite', 'like', 'dislike', 'remove']} data={{model: 'like', id: item.id, key: item.cuid, object: item.object, type: item.type, label: item.cuid}} />
            </div>            
          ...
          <ListContainer>
            {groupedData[type]?.map((item) => (
              <ListItem key={item.id}>
                ...
                <div className="flex flex-none items-center gap-x-4">
                    <ActionsComponent actions={['remove']} data={{model: 'like', id: item.id, key: item.cuid, object: item.object, type: item.type, label: item.cuid}} />
                </div>
              </ListItem>
            )) ?? null}
          </ListContainer>
