...
export function AllPinned({userId}: AllPinnedProps) {
  const [AllPinned] = api.pin.getByUser.useSuspenseQuery(
    { createdBy: userId }
  );
  ...
  if (!AllPinned || AllPinned.length === 0) {
    return (
      <AlertComponent type={"info"} icon={false} title={"No Objects Pinned"} />
    );
  }

  // Group the data by 'type'
  const groupByType = (AllPinned: Pin[]) => {
    return AllPinned.reduce((acc: Record<string, Pin[]>, item) => {
      if (!acc[item.type]) {
        acc[item.type] = [];
      }
  ...
  const groupedData = groupByType(AllPinned);

  if (AllPinned && Object.keys(groupedData).length === 1) {
    return (
      <ListContainer>
        {AllPinned.map((item: Pin) => (
          <ListItem key={item.id}>
            ...
            <div className="flex flex-none items-center gap-x-4">
              <ActionsComponent actions={['favorite', 'like', 'dislike', 'remove']} data={{model: 'pin', id: item.id, key: item.cuid, object: item.object, type: item.type, label: item.cuid}} />
            </div>            
          ...
          <ListContainer>
            {groupedData[type]?.map((item) => (
              <ListItem key={item.id}>
                ...
                <div className="flex flex-none items-center gap-x-4">
                    <ActionsComponent actions={['remove']} data={{model: 'pin', id: item.id, key: item.cuid, object: item.object, type: item.type, label: item.cuid}} />
                </div>
              </ListItem>
            )) ?? null}
          </ListContainer>
